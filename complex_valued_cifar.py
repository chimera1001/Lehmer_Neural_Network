...
def train_and_evaluate_cifar_improved(dataset="cifar10", units=128, epochs=100, batch_size=64):
    ...
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Conv2D(128, kernel_size=(3, 3), activation='relu', input_shape=(32, 32, 3)),
    ...
    print(f"Test Accuracy on {dataset.upper()}: {test_accuracy:.2f}")
    print(f"Test Loss on {dataset.upper()}: {test_loss:.2f}")
    
    model.summary()


# Train and evaluate on CIFAR-10
train_and_evaluate_cifar_improved(dataset="cifar10", units=24, epochs=5, batch_size=128)

# Train and evaluate on CIFAR-100
# train_and_evaluate_cifar_improved(dataset="cifar100", units=128, epochs=50, batch_size=64)
